...
class Orchestrator:
    ...
    def __init__(self, config_path: str, bot_filter: list = None):
        ...
        # Inicializa o estado com base no horário atual
        now = datetime.now()
        if self.is_weekend_shutdown(now):
            self._state = "STANDBY"
            print("[ORCH] Iniciado em modo STANDBY (Fim de Semana).")
        else:
            self._state = "ACTIVE"
            print("[ORCH] Iniciado em modo ACTIVE.")

        # Inicializa o rastreamento de logs existentes para todos os bots carregados
        for bot in self.bots.values():
            bot.initialize_log_tracking()

    def is_weekend_shutdown(self, dt: datetime) -> bool:
        """Verifica se o momento atual está na janela de standby do fim de semana (Sexta 20h a Segunda 9h)."""
        wd = dt.weekday()  # Segunda=0, Terça=1, ..., Sexta=4, Sábado=5, Domingo=6
        if wd == 4:  # Sexta
            return dt.hour >= 20
        elif wd in (5, 6):  # Sábado, Domingo
            return True
        elif wd == 0:  # Segunda
            return dt.hour < 9
        return False
    ...
    def _cmd_status_all(self):
        if self._state == "STANDBY":
            lines = [
                "💤 <b>MODO STANDBY (Fim de Semana)</b>",
                "━━━━━━━━━━━━━━━━━━━━━",
                "Estado: 💤 Adormecido (Mercado Forex Fechado)",
                "Todos os robôs estão desligados graciosamente.",
                "O orquestrador acordará automaticamente na segunda-feira às 09:00.",
                "━━━━━━━━━━━━━━━━━━━━━"
            ]
            self._send_telegram("\n".join(lines))
            return
    ...
    def _cmd_status_bot(self, query: str):
        ...
        if self._state == "STANDBY":
            self._send_telegram(
                ...
                f"━━━━━━━━━━━━━━━━━━━━━\n"
                f"🆔 ID: {bot.id}\n"
                f"📍 Status: STANDBY (Fim de Semana)\n"
                f"O orquestrador está em modo standby e o robô está adormecido graciosamente até segunda-feira às 09:00."
            )
            return
    ...
    def _cmd_restart_bot(self, query: str):
        if self._state == "STANDBY":
            self._send_telegram("⚠️ <b>Comando Recusado</b>\nO orquestrador está em modo <b>STANDBY</b> devido ao final de semana. Não é permitido reiniciar robôs neste período.")
            return
    ...
    def _cmd_start_bot(self, query: str):
        if self._state == "STANDBY":
            self._send_telegram("⚠️ <b>Comando Recusado</b>\nO orquestrador está em modo <b>STANDBY</b> devido ao final de semana. Não é permitido iniciar robôs neste período.")
            return
    ...
    def run(self):
        ...
        now = datetime.now()
        if self._state == "STANDBY":
            print("\n[ORCH] 🎛️  Orquestrador iniciado diretamente em modo STANDBY (Fim de Semana).")
            self._send_telegram("🎛️ <b>ORQUESTRADOR INICIADO</b> em modo <b>STANDBY</b> (Mercado Forex Fechado).")
            self._log_event("ORCH_START_STANDBY", detail="Iniciado no final de semana")
        else:
            self.start_all()
        ...
        print(f"\n[ORCH] 🎛️  Loop de monitoramento ativo em modo {self._state}. CTRL+C para sair.\n")

        try:
            while self._running:
                now = datetime.now()
                in_shutdown_window = self.is_weekend_shutdown(now)

                if self._state == "ACTIVE" and in_shutdown_window:
                    # Transiciona para STANDBY
                    print("\n[ORCH] 🕒 Sexta-feira 20h atingida. Transicionando para modo STANDBY...")
                    self._log_event("STANDBY_ENTER", detail="Sexta 20h atingida")
                    self._send_telegram("💤 <b>STANDBY (Fim de Semana)</b>\nHora de dormir! Parando todos os robôs para o fechamento do Forex...")
                    self.stop_all()
                    self._state = "STANDBY"
                    
                elif self._state == "STANDBY" and not in_shutdown_window:
                    # Transiciona para ACTIVE
                    print("\n[ORCH] 🕒 Segunda-feira 09h atingida. Transicionando para modo ACTIVE...")
                    self._log_event("STANDBY_EXIT", detail="Segunda 09h atingida")
                    self._send_telegram("🌅 <b>BOM DIA!</b>\nHora de acordar! Iniciando todos os robôs para a abertura do Forex...")
                    self._state = "ACTIVE"
                    self.start_all()

                # Executa saúde apenas se estiver ativo
                if self._state == "ACTIVE":
                    self._health_check()

                # Escuta comandos do Telegram e envia relatórios em ambos os modos
